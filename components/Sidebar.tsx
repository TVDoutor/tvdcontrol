...
const Sidebar: React.FC<SidebarProps> = ({ isOpen = false, onClose }) => {
  ...
  const handleProfileClick = () => {
      ...
      if (user?.id) {
        navigate('/users', { state: { targetUserId: user.id, editMode: true } });
      } else {
        navigate('/users');
      }
      if (onClose) onClose();
        ...
        <div className="flex flex-col h-full p-4 justify-between">
          ...
          >
            <div 
              ...
              style={{
                backgroundImage: `url("${user?.avatar || 'https://lh3.googleusercontent.com/aida-public/AB6AXuBVaALDdkXZEakGv-aNod9NCb-2GkYppHOzw9oubYd8sdoJNsvv4JM0DHkgBRqOy64FolHrmbTZGX9lEZqT8T9dkS_4I2XnqNYUoVS1PWJeUwM0Uoy7bpZiRa55Uyg4e3mLYg2X3YfCRkBakOiaMCCqjkBhEQFAyNjUYt-pY5j0YnI7GHnVnW9qf4C9tPi6ESIYQoaMufD37bvDhpdeDd11IZQup2XqG9mR2vhdVtNXhJ5CZLtYdlSeYVar1DPILY-ifbocYRtCa5A'}")`,
              }}
            ></div>
            <div className="flex flex-col min-w-0">
              <p className="text-sm font-semibold truncate text-text-main-light dark:text-text-main-dark group-hover:text-primary transition-colors">
                {user?.name || 'Usuário'}
              </p>
